...
function DialogSearchFilter(){
    this.data = null;
}


var dialogSearchFilter = new DialogSearchFilter();
...
var LinkDialog_LinkDialog = /*#__PURE__*/function () {
  function LinkDialog(context) {
    ...
    function http(type, url, formData){
  ...
  LinkDialog_createClass(LinkDialog, [{
    key: "initialize",
    value: function initialize() {
        http('GET', '/get-all-product-for-summernote', {}).then((e)=>{
            var d = JSON.parse(e);
            d = Object.entries(d, []).map(([key, value]) => ({key,value}));
            dialogSearchFilter.data = d;
            var aHtml = ``;
            d.forEach(element => {
                aHtml += `<div class="form-check form-check-inline">
                <input class="form-check-input CreateLinkRadio" type="radio" name="searchDialogRadio" id="CreateLinkRadioId-${this.options.id}-${element.key}" value="${element.key}" optionId="${this.options.id}">
                <label class="form-check-label" for="inlineRadio${element.key}">${element.key}</label>
              </div>`;
            });
            ...
            var body = [
                ...
                '</div>',
                '<div style="padding-top: 3%;">', ''+aHtml+'', '</div>',
                '<div style="padding: 3% 0 5% 0;display:none;" id="CreateLinkSelectArea-'+this.options.id+'"><select class="form-control select CreateLinkSelect" id="CreateLinkSelectId-'+this.options.id+'" optionId="'+this.options.id+'">', '', '</select></div>',
                '<div class="form-group note-form-group">',
                ...
                '</div>', !this.options.disableLinkTarget ? external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<div/>').append(this.ui.checkbox({
            ...
            var buttonClass = 'btn btn-primary note-btn note-btn-primary note-link-btn CreateLinkBtn';
            var footer = "<input type=\"button\" href=\"#\" optionId='"+this.options.id+"' id='CreateLinkBtnId-"+this.options.id+"' class=\"".concat(buttonClass, "\" value=\"").concat(this.lang.link.insert, "\" disabled>");
      ...
      return external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default.a.Deferred(function (deferred) {
        ...
        _this.ui.onDialogShown(_this.$dialog, function () {
          ...
          $linkBtn.one('click', function (event) {
            event.preventDefault();
            var optionId = $($linkBtn).attr('optionId');
            deferred.resolve({
              range: linkInfo.range,
              url: $(`#CreateLinkSelectId-${optionId}`).val(),
              text: $linkText.val(),
...
function CreatLinkDilogeSetting(){
    $(".CreateLinkRadio").unbind('change');
    $(".CreateLinkSelect").unbind('change');
    $(".CreateLinkRadio").on("change", function(){
        var optionId = $(this).attr('optionid');
        var data = dialogSearchFilter.data;
        var val = $(this).val();

        $(`#CreateLinkSelectId-${optionId}`).html('');
        $(`#CreateLinkSelectArea-${optionId}`).show();
        $(`#CreateLinkSelectId-${optionId}`).chosen({disable_search_threshold: 10});

        data = data.find(x=>x.key==val);
        $(`#CreateLinkSelectId-${optionId}`).append(`<option value=""></option>`);
        if(data.value.length>0){
            data.value.forEach(element => {
                $(`#CreateLinkSelectId-${optionId}`).append(`<option value="${element.url}">${element.name}</option>`);
            });
        }
        $(`#CreateLinkSelectId-${optionId}`).chosen({disable_search_threshold: 10});
        $(`#CreateLinkSelectId-${optionId}`).trigger("chosen:updated");
        $(`#CreateLinkSelectId_${optionId}_chosen`).css("width", "100%");
    });
    $(".CreateLinkSelect").on("change", function(){
        var optionId = $(this).attr('optionid');
        $(`#CreateLinkBtnId-${optionId}`).removeClass('disabled').removeAttr('disabled');
    });
}

function uploadSummernoteImage(image, element){
    ...
    $.ajax({
        ...
        url: '/upload-image-for-summernote',
        cache: false,
        contentType: false,
        processData: false,
        data: data,
...
function deleteSummnernoteImage(src){
    var data = new FormData();
    data.append("src", src);
    $.ajax({
        headers: {
        ...
        url: '/delete-image-for-summernote',
        cache: false,
        contentType: false,
        processData: false,
        data: data,
